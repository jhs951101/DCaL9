function [r, time, amplitude_response, phase_response, freq] = Function_Channel_Filter(x, gain, noise_power, f_cutoff, fs)
...
freq = fs/(2*fpoints) * (0:fpoints-1);  % Frequency vector;
H = freqz(B, A, fpoints);               % Frequency response;
amplitude = abs(H);
amplitude_response = 20*log10(amplitude);

phase = angle(H);
phase1 = unwrap(phase);
phase_response = phase1*180/pi;              % Phase in degree;
